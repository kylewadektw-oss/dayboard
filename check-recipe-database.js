...
async function checkRecipeDatabase() {
  ...
  try {
    ...
    const {
      ...
      error: recipesError,
      count
    } = await supabase.from('recipes').select('*', { count: 'exact' }).limit(5);

    console.log('📊 Recipe Database Status:');
    if (recipesError) {
      console.log('❌ Error accessing recipes table:', recipesError.message);
      console.log(
        "💡 This might mean the table doesn't exist or permissions are missing"
      );
    } else {
      ...
      recipes?.forEach((recipe, index) => {
        console.log(
          `${index + 1}. ${recipe.title} (${recipe.cuisine || 'Unknown cuisine'})`
        );
      });
    }
    ...
    try {
      ...
      if (!isValidKey) {
        console.log(
          '❌ API key appears to be a placeholder. Please set a real Spoonacular API key.'
        );
      }
    } catch (apiError) {
    ...
    if (count === 0) {
      console.log(
        '1. 📥 Database is empty - use the "Fetch Recipes" button to populate it'
      );
      console.log(
        '2. 🔍 Try fetching recipes with different search terms (e.g., "chicken", "pasta", "vegetarian")'
      );
      console.log(
        "3. 🎯 Use the Recipe Library's fetch functionality to get started"
      );
    } else {
      console.log(
        '1. ✅ Database has recipes - the Recipe Library should display them'
      );
      console.log(
        "2. 🔍 If recipes aren't showing, check the component's database query"
      );
