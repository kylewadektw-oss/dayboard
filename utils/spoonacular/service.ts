...
class SpoonacularService {
  ...
  private initializeApiKey() {
    if (this.apiKey === null) {
      ...
      if (!this.apiKey) {
        throw new Error(
          'Spoonacular API key not found in environment variables'
        );
      }
    }
  }
  ...
  private normalizeRecipeData(rawData: SpoonacularRecipe) {
    const ingredients =
      rawData.extendedIngredients?.map((ing) => ({
        name: ing.name,
    ...
    const instructions =
      rawData.analyzedInstructions?.[0]?.steps.map((step) => step.step) ||
    ...
    return {
      ...
      nutrition: rawData.nutrition
        ? {
            calories:
              rawData.nutrition.nutrients.find((n) => n.name === 'Calories')
                ?.amount || 0,
            protein:
              rawData.nutrition.nutrients.find((n) => n.name === 'Protein')
                ?.amount || 0,
            carbs:
              rawData.nutrition.nutrients.find(
                (n) => n.name === 'Carbohydrates'
              )?.amount || 0,
            fat:
              rawData.nutrition.nutrients.find((n) => n.name === 'Fat')
                ?.amount || 0
          }
        : undefined
    };
  ...
  async searchAndSubmitRecipes(params: {
    ...
    try {
      ...
      console.log(
        `🔍 Found ${searchResults.results.length} recipes for query: ${params.query}`
      );
      ...
      // Submit to queue
      const submissionResults = await this.bulkSubmitRecipesToQueue(
        recipeIds,
        params.userId
      );
