...
export function optimizedStringify(obj: unknown, space?: number): string {
  try {
    ...
    if (result.length > 100 * 1024) {
      console.warn(
        `Large string detected (${(result.length / 1024).toFixed(1)}KB). Consider using Buffer optimization.`
      );
    }
...
export class SerializationProfiler {
  ...
  static time<T>(operation: string, fn: () => T): T {
    ...
    if (duration > 100) {
      console.warn(
        `Slow serialization operation: ${operation} took ${duration.toFixed(2)}ms`
      );
    }
  ...
  static getStats(
    operation: string
  ): { avg: number; max: number; count: number } | null {
