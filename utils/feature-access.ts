...
export function createFeatureAccessManager(supabase: SupabaseClient): FeatureAccessManager {
...
export const featureAccessManager = {
  ...
  updateFeatureAccess: (featureName: string, role: string, available: boolean) => {
    console.log(`Mock: Updating ${featureName} for ${role}: ${available}`);
    return Promise.resolve(true);
  },
  batchUpdateFeatureAccess: (householdId: string, updates: unknown[]) => {
    console.log(`Mock: Batch updating features for household ${householdId}`, updates);
    return Promise.resolve(true);
  },
  setupDefaultFeatureAccess: (householdId: string) => {
    console.log(`Mock: Setting up default features for household ${householdId}`);
    return Promise.resolve(true);
  }
};
...
export const checkFeatureAccess = (userId: string, featureName: string) => {
  console.log(`Mock: Checking feature access for user ${userId} to ${featureName}`);
  return Promise.resolve(true);
};
export const checkMultipleFeatureAccess = (userId: string, featureNames: string[]) => {
  console.log(`Mock: Checking multiple feature access for user ${userId}`, featureNames);
  const results: Record<string, boolean> = {};
  featureNames.forEach(name => { results[name] = true; });
