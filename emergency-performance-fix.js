  ...
  try {
    ...
    if (profiles && profiles.length > 0) {
      ...
      if (uncachedProfile) {
        ...
        console.log(`🚀 Cache improvement: ${uncachedTime - cachedTime}ms faster`);
      }
    }
    ...
    if (!logError) {
      console.log(`📊 Total application logs: ${logCount?.length || 'Unknown'}`);
    ...
    console.log('CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_user_id_fast');
    console.log('ON profiles(user_id) WHERE user_id IS NOT NULL;');
    console.log('');
    console.log('-- 2. Optimized log queries');
    console.log('CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_logs_timestamp_recent');
    console.log('ON application_logs(timestamp DESC) WHERE timestamp > (NOW() - INTERVAL \'7 days\');');
    console.log('');
