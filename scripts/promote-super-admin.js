...
async function main() {
  ...
  if (!email || !email.includes('@')) {
    console.error('Provide target email. Example: node scripts/promote-super-admin.js user@example.com');
    process.exit(1);
  }
  ...
  if (upsertError) { console.error('Upsert failed:', upsertError); process.exit(1); }

  // Verify
  const { data: verify, error: verifyError } = await supabase.from('profiles').select('id, role').eq('id', user.id).single();
  if (verifyError) { console.error('Verification failed:', verifyError); process.exit(1); }
