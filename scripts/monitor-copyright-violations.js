...
const CODE_SIGNATURES = [
  ...
  'BentLo Labs LLC (developer@bentlolabs.com)',
...
function searchForSignature(signature, callback) {
  ...
  console.log(
    `🔗 Manual search URL: https://www.google.com/search?q=${searchQuery}`
  );
...
function generateMonitoringReport() {
  const report = {
    ...
    manual_search_urls: CODE_SIGNATURES.map(
      (sig) =>
        `https://www.google.com/search?q=${encodeURIComponent(`"${sig}"`)}`
    ),
    github_search_urls: CODE_SIGNATURES.map(
      (sig) =>
