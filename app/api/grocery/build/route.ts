...
function categorizeAisle(name: string): string {
  ...
  if (
    /(lettuce|onion|garlic|pepper|tomato|cilantro|cucumber|spinach|apple|banana|lime|lemon|avocado|herb)/.test(
      n
    )
  )
    return 'Produce';
  ...
  if (/(frozen|ice cream|peas \(frozen\)|corn \(frozen\))/.test(n))
    return 'Frozen';
  if (
    /(rice|pasta|beans|canned|broth|stock|tomato sauce|spice|salt|sugar|flour|oil)/.test(
      n
    )
  )
    return 'Pantry';
...
function prettyUnit(qty: number, unit: string) {
  ...
  if (unit === 'ml') {
    if (
      qty >= 480 &&
      Math.abs(qty / ML_PER_CUP - Math.round(qty / ML_PER_CUP)) < 0.05
    )
      return `${Math.round(qty / ML_PER_CUP)} cup(s)`;
    if (qty >= ML_PER_TBSP && qty % ML_PER_TBSP === 0)
      return `${qty / ML_PER_TBSP} tbsp`;
    if (qty >= ML_PER_TSP && qty % ML_PER_TSP === 0)
      return `${qty / ML_PER_TSP} tsp`;
    return `${Math.round(qty)} ml`;
  }
  if (unit === 'g') {
    if (
      qty >= 900 &&
      Math.abs(qty / G_PER_LB - Math.round(qty / G_PER_LB)) < 0.05
    )
      return `${(qty / G_PER_LB).toFixed(1)} lb`;
    if (
      qty >= G_PER_OZ &&
      Math.abs(qty / G_PER_OZ - Math.round(qty / G_PER_OZ)) < 0.05
    )
...
function toPlainText(
  groups: Record<string, ConsolidatedItem[]>,
  staples: ConsolidatedItem[]
) {
  const lines: string[] = [];
  for (const aisle of Object.keys(groups).sort()) {
    lines.push(`\n## ${aisle}`);
    for (const it of groups[aisle])
      lines.push(`- ${it.name} — ${prettyUnit(Math.round(it.qty), it.unit)}`);
  }
  if (staples.length) {
    lines.push(`\n## Staples`);
    for (const it of staples)
      lines.push(`- ${it.name} — ${prettyUnit(Math.round(it.qty), it.unit)}`);
  }
  return lines.join('\n');
}
...
function toCSV(
  ...
  const push = (it: ConsolidatedItem) =>
    rows.push([it.aisle, it.name, String(Math.round(it.qty)), it.unit]);
...
async function fetchIngredientsForMealPlan(
  ...
  const { data: items, error } = await supabase
    ...
    .select(
      'recipe_id, servings, recipes:recipe_id(recipe_ingredients(name,quantity,unit,aisle))'
    )
  ...
  for (const row of items || []) {
    ...
    const ingredients = Array.isArray(row.recipes)
      ? row.recipes.flatMap((r) => r.recipe_ingredients || [])
      : [];
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (Array.isArray(body.ingredients)) {
      ...
      if (body.mealPlanId) {
        ingredients = await fetchIngredientsForMealPlan(
          supabase,
          body.mealPlanId
        );
        ...
        return NextResponse.json(
          { error: 'Provide `ingredients[]`, `mealPlanId`, or `recipes[]`.' },
          { status: 400 }
        );
      }
    }
    ...
    const message =
      e instanceof Error ? e.message : 'Failed to build grocery list';
