...
export async function GET(request: NextRequest) {
  try {
    ...
    if (filters.maxCookTime) {
      filteredRecipes = filteredRecipes.filter(recipe =>
        recipe.cook_time <= filters.maxCookTime!
      );
    }
    ...
    if (filters.quickOnly) {
      filteredRecipes = filteredRecipes.filter(recipe =>
        (recipe.prep_time + recipe.cook_time) <= 20
      );
    }
    ...
    const transformedRecipes = paginatedRecipes.map(recipe => ({
      ...
      diet_types: recipe.dietary_tags.filter(tag => 
        ['vegetarian', 'vegan', 'gluten_free', 'dairy_free', 'keto', 'paleo', 'low_carb'].includes(tag)
      ),
    ...
    if (filters.diets?.length) appliedFilters.push(`${filters.diets.length} diet restrictions`);
    if (filters.allergensExclude?.length) appliedFilters.push(`${filters.allergensExclude.length} allergen exclusions`);
    if (filters.cuisines?.length) appliedFilters.push(`${filters.cuisines.length} cuisines`);
    if (filters.maxCookTime) appliedFilters.push(`≤${filters.maxCookTime} min cook time`);
    if (filters.minRating) appliedFilters.push(`≥${filters.minRating} star rating`);
    ...
    const response = {
      ...
      pagination: {
        ...
        offset,
        limit,
        hasMore: paginatedRecipes.length === limit && (offset + limit) < filteredRecipes.length
      },
      filters: {
        applied: filters,
  ...
  try {
    ...
    const response = {
      ...
      pagination: {
        ...
        hasMore: paginatedRecipes.length === pagination.limit && 
