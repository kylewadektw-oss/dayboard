...
export async function GET(req: NextRequest) {
  try {
    ...
    const from = url.searchParams.get("from") ?? new Date(Date.now() - 7 * 864e5).toISOString();
    const to = url.searchParams.get("to") ?? new Date(Date.now() + 60 * 864e5).toISOString();
    ...
    try {
      ...
      console.log("Calendar view not available, using mock data for iCal export");
      ...
      events = [
        ...
        },
        {
          ...
          location: null,
          start_ts: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000 + 7.5 * 60 * 60 * 1000).toISOString(),
          end_ts: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000 + 8.5 * 60 * 60 * 1000).toISOString(),
          all_day: false,
          color: '#22c55e'
        }
      ].filter(event => {
