...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (
        errorMessage.includes('calendar_events') ||
        errorMessage.includes('does not exist')
      ) {
        console.log(
          'Calendar tables not deployed yet, simulating event creation'
        );
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        console.log(
          'Calendar tables not deployed yet, returning mock manual events'
        );
        ...
        const mockEvents = [
          {
            ...
            end_ts: new Date(
              Date.now() + 5 * 24 * 60 * 60 * 1000 + 21 * 60 * 60 * 1000
            ).toISOString(),
            all_day: false,
            color: '#8b5cf6',
            created_at: new Date().toISOString()
