...
export default function DevToolsDashboard() {
  ...
  const tabs: TabData[] = [
    ...
    { id: 'performance', label: 'Performance', icon: '⚡', count: monitorData?.performance ? Object.keys(monitorData.performance.coreWebVitals).length : 0 },
    { id: 'network', label: 'Network', icon: '🌐', count: monitorData?.network?.totalRequests || 0 },
    { id: 'console', label: 'Console', icon: '💬', count: monitorData?.console?.messages?.length || 0 },
    { id: 'security', label: 'Security', icon: '🔒', count: monitorData?.security?.total || 0 },
    { id: 'memory', label: 'Memory', icon: '💾' },
    { id: 'interactions', label: 'Interactions', icon: '👆', count: monitorData?.interactions?.total || 0 }
  ];
          ...
          <div className="bg-white border-b border-gray-200 px-6 py-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600">Comprehensive browser performance and behavior monitoring</p>
              </div>
          ...
          <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="space-y-2 text-sm">
                      {monitorData.performance.coreWebVitals.lcp && (
                        <div className="flex justify-between">
                          <span>LCP:</span>
                          <span className={getPerformanceScoreColor('lcp', monitorData.performance.coreWebVitals.lcp)}>
                            {formatDuration(monitorData.performance.coreWebVitals.lcp)}
                          </span>
                        </div>
                      )}
                      {monitorData.performance.coreWebVitals.fid && (
                        <div className="flex justify-between">
                          <span>FID:</span>
                          <span className={getPerformanceScoreColor('fid', monitorData.performance.coreWebVitals.fid)}>
                            {formatDuration(monitorData.performance.coreWebVitals.fid)}
                          </span>
                        </div>
                      )}
                      {monitorData.performance.coreWebVitals.cls && (
                        <div className="flex justify-between">
                          <span>CLS:</span>
                          <span className={getPerformanceScoreColor('cls', monitorData.performance.coreWebVitals.cls)}>
                            {monitorData.performance.coreWebVitals.cls.toFixed(3)}
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span>Failed:</span>
                        <span className="text-red-600">{monitorData.network.failedRequests}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Avg Time:</span>
                        <span className="text-blue-600">{formatDuration(monitorData.network.averageResponseTime)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Success Rate:</span>
                        <span className="text-green-600">
                          {monitorData.network.totalRequests > 0 
                            ? Math.round(((monitorData.network.totalRequests - monitorData.network.failedRequests) / monitorData.network.totalRequests) * 100)
                            : 100}%
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-2xl font-bold text-purple-600">
                        {monitorData.memory.current ? formatBytes(monitorData.memory.current.usedJSHeapSize) : 'N/A'}
                      </span>
                    </div>
                    {monitorData.memory.current && (
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span>Used:</span>
                          <span>{formatBytes(monitorData.memory.current.usedJSHeapSize)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Total:</span>
                          <span>{formatBytes(monitorData.memory.current.totalJSHeapSize)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Pressure:</span>
                          <span className={`font-medium ${
                            monitorData.memory.current.pressure === 'critical' ? 'text-red-600' :
                            monitorData.memory.current.pressure === 'high' ? 'text-orange-600' :
                            monitorData.memory.current.pressure === 'medium' ? 'text-yellow-600' : 'text-green-600'
                          }`}>
                  ...
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span>Critical:</span>
                        <span className="text-red-600">{monitorData.security.critical}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Total Events:</span>
                        <span className="text-orange-600">{monitorData.security.total}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Status:</span>
                        <span className={monitorData.security.critical > 0 ? 'text-red-600' : 'text-green-600'}>
                          {monitorData.security.critical > 0 ? 'Issues' : 'Secure'}
                        </span>
                      </div>
                    </div>
                  </div>
                        ...
                        {monitorData.network.requests.slice(-5).map((request: NetworkRequest, index: number) => (
                          <div key={index} className="flex items-center justify-between text-sm p-2 bg-gray-50 rounded">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="truncate max-w-xs">{new URL(request.url).pathname}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className={request.failed ? 'text-red-600' : 'text-green-600'}>{request.status}</span>
                          ...
                          <div key={index} className="text-sm p-2 bg-gray-50 rounded">
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`text-xs px-2 py-0.5 rounded ${
                                message.level === 'error' ? 'bg-red-100 text-red-800' :
                                message.level === 'warn' ? 'bg-yellow-100 text-yellow-800' :
                            ...
                            <p className="text-gray-800 truncate">{message.message}</p>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {Object.entries(monitorData.performance.coreWebVitals).map(([metric, value]) => (
                      <div key={metric} className="text-center">
                        <div className={`text-3xl font-bold mb-2 ${getPerformanceScoreColor(metric, value as number)}`}>
                          {metric === 'cls' ? (value as number).toFixed(3) : formatDuration(value as number)}
                        </div>
                ...
                {monitorData.performance.navigation && (
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <h3 className="font-semibold text-gray-900 mb-4">🚀 Navigation Timing</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {Object.entries(monitorData.performance.navigation).map(([phase, time]) => (
                ...
                {/* Custom Timings */}
                {Object.keys(monitorData.performance.customTimings).length > 0 && (
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <h3 className="font-semibold text-gray-900 mb-4">⏱️ Custom Timings</h3>
                    <div className="space-y-2">
                      {Object.entries(monitorData.performance.customTimings).map(([name, duration]) => (
                        <div key={name} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                          <span className="font-medium text-gray-800">{name}</span>
                          <span className="font-mono text-blue-600">{formatDuration(duration as number)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                  <div className="p-6 border-b border-gray-200">
                    <h3 className="font-semibold text-gray-900">🌐 Network Requests</h3>
                    <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
                      <span>Total: {monitorData.network.totalRequests}</span>
                      <span>Failed: {monitorData.network.failedRequests}</span>
                      <span>Avg Response: {formatDuration(monitorData.network.averageResponseTime)}</span>
                    </div>
                  </div>
                          ...
                          <tr key={index} className={request.failed ? 'bg-red-50' : ''}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              <span className={`font-medium ${
                                ...
                                request.status >= 300 && request.status < 400 ? 'text-yellow-600' :
                                'text-red-600'
                              }`}>
                                {request.status}
                              </span>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {request.cached && <span className="text-green-600">Cached</span>}
                              {request.failed && <span className="text-red-600">Failed</span>}
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {activeTab === 'console' && (
              <div className="space-y-6">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                  <div className="p-6 border-b border-gray-200">
                    <h3 className="font-semibold text-gray-900">💬 Console Messages</h3>
                    <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
                  ...
                  <div className="p-6 space-y-2">
                    {monitorData.console.messages.slice(-50).map((message: ConsoleMessage, index: number) => (
                      <div key={index} className={`p-3 rounded border-l-4 ${
                        ...
                        message.level === 'info' ? 'bg-blue-50 border-blue-400' :
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs px-2 py-0.5 rounded font-medium ${
                            message.level === 'error' ? 'bg-red-100 text-red-800' :
                            message.level === 'warn' ? 'bg-yellow-100 text-yellow-800' :
                            message.level === 'info' ? 'bg-blue-100 text-blue-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                            {message.level.toUpperCase()}
                          </span>
                          ...
                          <span className="text-gray-400 text-xs">
                            {new Date(message.timestamp + Date.now() - performance.now()).toLocaleTimeString()}
                          </span>
                        </div>
              ...
              <div className="space-y-6">
                {monitorData.security.events.length > 0 ? (
                      ...
                      {monitorData.security.events.map((event: SecurityEvent, index: number) => (
                        <div key={index} className={`p-4 rounded-lg border ${
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`px-2 py-1 text-xs font-medium rounded ${
                              event.blocked ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                            }`}>
                              {event.blocked ? 'BLOCKED' : 'ALLOWED'}
                            </span>
                          </div>
                          <div className="text-gray-800 text-sm mb-2">{event.details}</div>
                          {event.source && (
                            <div className="text-gray-600 text-xs">Source: {event.source}</div>
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                    ...
                    <p className="text-gray-600">Your application appears to be secure with no detected violations.</p>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="space-y-6">
                {monitorData.memory.current ? (
                  <>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                      <h3 className="font-semibold text-gray-900 mb-4">💾 Current Memory Usage</h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="text-center">
                          <div className="text-3xl font-bold text-purple-600 mb-2">
                            {formatBytes(monitorData.memory.current.usedJSHeapSize)}
                          </div>
                          <div className="text-sm font-medium text-gray-700">Used Heap Size</div>
                        </div>
                        <div className="text-center">
                          <div className="text-3xl font-bold text-blue-600 mb-2">
                            {formatBytes(monitorData.memory.current.totalJSHeapSize)}
                          </div>
                          <div className="text-sm font-medium text-gray-700">Total Heap Size</div>
                        </div>
                        <div className="text-center">
                          <div className="text-3xl font-bold text-gray-600 mb-2">
                            {formatBytes(monitorData.memory.current.jsHeapSizeLimit)}
                          </div>
                          <div className="text-sm font-medium text-gray-700">Heap Size Limit</div>
                        </div>
                      </div>
                      ...
                      <div className="mt-6">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-medium text-gray-700">Memory Pressure</span>
                      ...
                      {monitorData.memory?.current.leakSuspicion && (
                        <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-red-800">Potential Memory Leak Detected</span>
                          </div>
                          <p className="text-red-700 text-sm mt-1">
                            Memory usage has been consistently increasing. Consider investigating for memory leaks.
                          </p>
                        </div>
                      )}
                    </div>
                          ...
                          {monitorData.memory.history.map((snapshot: MemoryUsage, index: number) => (
                            <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                              ...
                              <div className="flex items-center gap-4 text-sm">
                                <span>Used: {formatBytes(snapshot.usedJSHeapSize)}</span>
                                <span className={`font-medium ${
                                  snapshot.trend === 'increasing' ? 'text-red-600' :
                                  snapshot.trend === 'decreasing' ? 'text-green-600' : 'text-gray-600'
                                }`}>
                                  {snapshot.trend === 'increasing' ? '↗️' : 
                                   snapshot.trend === 'decreasing' ? '↘️' : '➡️'}
                                </span>
                              </div>
                            </div>
                          ))}
                  ...
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">Memory Monitoring Unavailable</h3>
                    <p className="text-gray-600">Memory API is not supported in this browser.</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'interactions' && (
              <div className="space-y-6">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                  <h3 className="font-semibold text-gray-900 mb-4">👆 User Interactions</h3>
