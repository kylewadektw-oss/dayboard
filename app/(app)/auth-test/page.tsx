...
export default function AuthTestPage() {
  ...
  // Stable loading state management - prevent oscillation
  ...
  // Stabilize auth state to prevent loading oscillation
  useEffect(() => {
    if (!loading && (user !== undefined || profile !== undefined)) {
      // Set stable state once auth loading is complete and we have data
      setStableAuthState({ user, profile });
  ...
  // Use stable auth state when available
  const currentUser = stableAuthState?.user || user;
  const currentProfile = stableAuthState?.profile || profile;
  const showLoading = !initialLoadComplete || (loading && !stableAuthState);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto space-y-8">
              ...
              {currentProfile && (
                <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <div className="text-sm text-green-700">
                    <div>Display Name: {currentProfile.name || 'Not set'}</div>
                    <div>Role: {currentProfile.role || 'Not set'}</div>
                    <div>
                      Created:{' '}
                      {currentProfile.created_at
                        ? new Date(currentProfile.created_at).toLocaleString()
                        : 'Not set'}
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h3 className="font-medium text-gray-900 mb-2 text-green-600">
                ✅ Public Routes
              </h3>
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-2 text-red-600">
                🔒 Protected Routes
              </h3>
              <div className="space-y-1 text-sm text-gray-600">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6">
          ...
          <div className="space-y-2 text-sm text-amber-800">
            <div>
              1. <strong>Test Public Access:</strong> Visit the landing page (/)
              - should load without authentication
            </div>
            <div>
              2. <strong>Test Protection:</strong> Try accessing /dashboard -
              should redirect to /signin
            </div>
            <div>
              3. <strong>Test Authentication:</strong> Sign in with Google OAuth
            </div>
            <div>
              4. <strong>Test Access:</strong> After signing in, try accessing
              protected routes
            </div>
            <div>
              5. <strong>Run Tests:</strong> Use the &ldquo;Run Tests&rdquo;
              button to verify all functionality
            </div>
          </div>
        </div>
      </div>
