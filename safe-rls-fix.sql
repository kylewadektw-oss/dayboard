...
-- Drop ALL possible policy names for profiles table
...
-- Drop ALL possible policy names for households table
...
-- Drop ALL possible policy names for user_permissions table
...
CREATE POLICY "profiles_auth_read_v2" ON profiles
    FOR SELECT 
    USING (auth.uid() IS NOT NULL);

CREATE POLICY "profiles_auth_insert_v2" ON profiles
    FOR INSERT 
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "profiles_auth_update_v2" ON profiles
    FOR UPDATE 
    USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "profiles_auth_delete_v2" ON profiles
    FOR DELETE 
    USING (auth.uid() = user_id);

-- Households policies
CREATE POLICY "households_auth_read_v2" ON households
    FOR SELECT 
    USING (auth.uid() IS NOT NULL);

CREATE POLICY "households_auth_insert_v2" ON households
    FOR INSERT 
    WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "households_auth_update_v2" ON households
    FOR UPDATE 
    USING (auth.uid() IS NOT NULL)
    WITH CHECK (auth.uid() IS NOT NULL);

-- User permissions policies
CREATE POLICY "user_permissions_auth_read_v2" ON user_permissions
    FOR SELECT 
    USING (auth.uid() IS NOT NULL);

CREATE POLICY "user_permissions_auth_all_v2" ON user_permissions
    FOR ALL 
    USING (auth.uid() IS NOT NULL)
    WITH CHECK (auth.uid() IS NOT NULL);
