...
async function checkSettingsTables() {
  ...
  try {
    ...
    if (userError) {
      ...
      console.log(userSettings?.length ? userSettings : '📭 No user settings found');
    }
    ...
    if (householdError) {
      ...
      console.log(householdSettings?.length ? householdSettings : '📭 No household settings found');
    }

    // Check the RPC function
    console.log('\n🔧 Testing get_user_settings_tabs function...');
    const { data: rpcData, error: rpcError } = await supabase
      .rpc('get_user_settings_tabs', { user_id_param: 'test' });
