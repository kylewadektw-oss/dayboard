...
async function updateTriggerFunction() {
  console.log('🔧 Updating profile completion trigger function to remove notification requirement...');
...
async function updateTrigger() {
  console.log('🔧 Updating profile completion trigger (removing notification_preferences)...');
...
async function testUpdatedCalculation() {
  console.log('🧪 Testing updated calculation without notification requirement...');
  
  try {
    // Get first profile
    const { data: profiles } = await supabase
      .from('profiles')
      .select('id, profile_completion_percentage, name, phone_number, date_of_birth, bio, timezone, language, dietary_preferences, allergies, avatar_url, notification_preferences')
      .limit(1);
    ...
    console.log(`📊 Expected percentage: ${Math.round(filledFields.length * 100 / 9)}%`);
    console.log(`📊 Actual percentage: ${profile.profile_completion_percentage}%`);
...
async function main() {
  console.log('🚀 Removing notification requirement from profile completion...\n');
  ...
  console.log('🎯 Profile completion is now based on 9 criteria instead of 10.');
  console.log('📋 Criteria: name, phone, birthday, bio, timezone, language, dietary preferences, allergies, avatar');
}
