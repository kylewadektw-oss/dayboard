...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const profileFetchCache = new Map<string, Promise<Profile | null>>();
  ...
  const fetchProfile = async (userId: string) => {
    ...
    // Return cached promise if already fetching for this user
    if (profileFetchCache.has(userId)) {
      console.log('� Using cached profile fetch for:', userId);
    ...
    const fetchPromise = (async () => {
      try {
        ...
        console.log('ℹ️ Profile fetch failed - treating as new user:', error);
        return null;
      } finally {
    ...
    profileFetchCache.set(userId, fetchPromise);
    return fetchPromise;
  };
  ...
  useEffect(() => {
    ...
    const getInitialSession = async () => {
      try {
        ...
        if (error) {
          ...
          // Skip profile fetch in AuthContext to prevent duplicate requests
          // Profile will be fetched by individual pages as needed
          if (session?.user) {
            console.log('� User authenticated in AuthContext:', session.user.id);
            // Don't fetch profile here - let the profile page handle it
          } else {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (session?.user) {
          // Skip duplicate profile fetch - let pages handle their own data
          console.log('🔄 Auth state change - user present:', session.user.id);
        } else {
