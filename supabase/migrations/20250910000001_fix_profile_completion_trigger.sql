-- Fix profile completion trigger to use correct column names
-- The original trigger used incorrect column names (full_name vs name, dietary_restrictions vs dietary_preferences)
...
BEGIN
  -- 1. Name (name OR preferred_name) - FIXED: was using full_name
  ...
  -- 7. Dietary preferences array present - FIXED: was using dietary_restrictions
...
-- Drop and recreate trigger with correct column names
...
-- Backfill existing rows with corrected calculation
UPDATE profiles SET profile_completion_percentage = (
  ...
  SELECT (
    (
      (CASE WHEN coalesce(nullif(trim(name),''), nullif(trim(preferred_name),'')) IS NOT NULL THEN 1 ELSE 0 END) +
      ...
      (CASE WHEN dietary_preferences IS NOT NULL AND array_length(dietary_preferences,1) > 0 THEN 1 ELSE 0 END) +
...
-- Ensure constraint is still valid
...
-- Comment for clarity
COMMENT ON FUNCTION public.set_profile_completion() IS 'Calculates profile completion percentage based on 10 criteria: name, phone, birthday, bio, timezone, language, dietary_preferences, allergies, avatar, and notification_preferences';
