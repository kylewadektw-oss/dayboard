...
const EntertainmentWidget: React.FC = memo(() => {
  const { featuredEvent, nowPlaying, familyActivity, weekendSuggestion } =
    entertainmentData;

  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-white bg-gradient-to-r from-purple-500 to-blue-600 px-3 py-1 rounded-lg tracking-wide">
          ENTERTAINMENT
        </h3>
        <Link
          href="/entertainment"
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600">
                  {familyActivity.date} at {familyActivity.time}
                </p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs font-medium text-green-600">
                {familyActivity.spotsLeft} spots left
              </div>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex-1">
              <h4 className="font-medium text-gray-900 text-sm">
                {weekendSuggestion.title}
              </h4>
              <p className="text-xs text-gray-600">
                {weekendSuggestion.frequency}
              </p>
              <p className="text-xs text-purple-600 mt-1">
                {weekendSuggestion.highlight}
              </p>
            </div>
          </div>
        </div>
