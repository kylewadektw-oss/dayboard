...
export const FinancialWidget = memo(() => {
  ...
  const topSavingsGoal = mockSavingsGoals.sort(
    (a, b) =>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-white bg-gradient-to-r from-green-500 to-emerald-600 px-3 py-1 rounded-lg tracking-wide">
          FINANCIAL OVERVIEW
        </h3>
      ...
      {/* Net Income */}
      <div
        className={`p-3 rounded-lg border mb-4 ${netIncome >= 0 ? 'bg-blue-50 border-blue-200' : 'bg-orange-50 border-orange-200'}`}
      >
        <div className="flex items-center justify-between mb-1">
          ...
          <span
            className={`text-xs ${netIncome >= 0 ? 'text-blue-600' : 'text-orange-600'}`}
          >
            {savingsRate.toFixed(1)}% saved
          </span>
        </div>
        <p
          className={`text-sm font-bold ${netIncome >= 0 ? 'text-blue-800' : 'text-orange-800'}`}
        >
      ...
      {topSavingsGoal && (
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 mb-4">
          ...
          <div className="space-y-1">
            ...
            <Progress
              value={
                (topSavingsGoal.currentAmount / topSavingsGoal.targetAmount) *
                100
              }
              className="h-2"
            />
