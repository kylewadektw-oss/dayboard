...
const BudgetAnalytics = () => {
  ...
  const getSpendingByCategory = () => {
    const categorySpending = new Map<
      string,
      { category: BudgetCategory; spent: number; budget: number }
    >();
    ...
    return Array.from(categorySpending.values()).sort(
      (a, b) => b.spent - a.spent
    );
  };
  ...
  const getSavingsProgress = () => {
    const totalTargetAmount = mockSavingsGoals.reduce(
      (sum, goal) => sum + goal.targetAmount,
      0
    );
    const totalCurrentAmount = mockSavingsGoals.reduce(
    ...
    return {
      ...
      totalCurrentAmount,
      progressPercentage,
      goalsOnTrack: mockSavingsGoals.filter(
        (goal) => (goal.currentAmount / goal.targetAmount) * 100 >= 50
      ).length
    };
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Insights and trends for your household finances
          </p>
        </div>
        <div className="flex space-x-2">
          <Select
            value={timeRange}
            onValueChange={(value) =>
              setTimeRange(value as 'month' | 'quarter' | 'year')
            }
          >
          ...
          <Select
            ...
            onValueChange={(value) =>
              setViewType(
                value as 'overview' | 'trends' | 'categories' | 'goals'
              )
            }
          >
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${budgetPerformance.performance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                >
                  {budgetPerformance.performance.toFixed(1)}%
                </p>
                <p className="text-xs text-gray-500">
                  {formatCurrency(budgetPerformance.remaining)}{' '}
                  {budgetPerformance.remaining >= 0 ? 'under' : 'over'}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  ...
                  {formatCurrency(categorySpending[0]?.spent || 0)}
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {savingsProgress.goalsOnTrack}/{mockSavingsGoals.length} on
                  track
                </p>
              </div>
                      ...
                      <div key={category.id} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            ...
                            <p
                              className={`text-xs ${isOverBudget ? 'text-red-600' : 'text-green-600'}`}
                            >
                ...
                {monthlyTrends.map((month) => (
                  ...
                  >
                    ...
                    <div className="flex space-x-4 text-sm">
                      ...
                      <div
                        className={`font-semibold ${month.savings >= 0 ? 'text-blue-600' : 'text-red-600'}`}
                      >
              ...
              <div className="space-y-4">
                {mockSavingsGoals.map((goal) => {
                  ...
                  const monthsRemaining = Math.ceil(
                    monthsToTarget / (1000 * 60 * 60 * 24 * 30)
                  );
