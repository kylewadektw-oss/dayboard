...
type Provider = 'google' | 'apple' | 'azure' | 'github';
...
export default function OauthSignIn() {
  ...
  const handleOAuthSignIn = async (provider: Provider) => {
    try {
      ...
      console.log(`🚀 ${provider} OAuth sign-in initiated`, {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          // Ensure proper scopes for each provider
          scopes: provider === 'google' ? 'email profile' : undefined
        }
      });

      console.log(`🔍 Supabase ${provider} OAuth response received`, {
        hasData: !!data,
        hasError: !!error,
      ...
      console.error(`❌ Critical ${provider} OAuth error`, error);
    ...
    <div className="space-y-3">
      {providers.map((provider) => (
        <button
          ...
          } focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-transparent disabled:opacity-50 disabled:cursor-not-allowed`}
        >
