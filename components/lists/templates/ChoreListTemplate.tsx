...
export default function ChoreListTemplate({ 
  ...
  const [newChoreFrequency, setNewChoreFrequency] = React.useState<'daily' | 'weekly' | 'monthly' | 'custom'>('weekly');
  const [newChorePoints, setNewChorePoints] = React.useState('10');
  const [newChoreDifficulty, setNewChoreDifficulty] = React.useState<'easy' | 'medium' | 'hard'>('medium');
  const [newChoreEstimatedTime, setNewChoreEstimatedTime] = React.useState('15');
  ...
  const maxPoints = items.reduce((sum, item) => sum + (item.details.points || 0), 0);
        ...
        <CardHeader>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div className="text-center">
              <div className="text-xl font-bold text-green-600">{completedChores}</div>
              <div className="text-sm text-gray-600">Completed</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-orange-600">{totalChores - completedChores}</div>
              <div className="text-sm text-gray-600">Remaining</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-purple-600">{maxPoints}</div>
          ...
          <div className="mt-4">
            <div className="flex justify-between text-sm text-gray-600 mb-2">
              ...
              <span>{totalChores > 0 ? Math.round((completedChores / totalChores) * 100) : 0}%</span>
            </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {roomChores.filter(chore => chore.checked).length} of {roomChores.length} complete
                      </p>
                    </div>
                  </div>
                  
                  {roomConfig && (
                    <Badge className={roomConfig.color}>
                      {roomName}
                    </Badge>
                  )}
                </div>
                    ...
                    <div key={chore.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3 flex-1">
                        ...
                        <div className="flex-1">
                          <div className={`font-medium ${chore.checked ? 'line-through text-gray-500' : ''}`}>
                            {chore.content}
                          </div>
                          ...
                          <div className="flex items-center space-x-4 mt-1 text-sm text-gray-600">
                            {chore.details.frequency && (
                              <span className="flex items-center">
                                ...
                                <span className="ml-1 capitalize">{chore.details.frequency}</span>
                              </span>
                            )}
                            
                            {chore.details.difficulty && (
                              <Badge className={DIFFICULTY_COLORS[chore.details.difficulty]}>
                                {getDifficultyIcon(chore.details.difficulty)} {chore.details.difficulty}
                              </Badge>
                            )}
                      ...
                      {chore.checked && (
                        <div className="flex items-center text-green-600">
                          ...
                          <span className="text-sm font-medium">+{chore.details.points || 0}</span>
                        </div>
                      )}
                    </div>
                  ))}
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-4 bg-yellow-50 rounded-lg">
              ...
              <div className="text-sm text-gray-600">Complete all daily chores</div>
            </div>
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <Star className="w-8 h-8 text-blue-600 mx-auto mb-2" />
              <div className="font-medium">Point Milestone</div>
              <div className="text-sm text-gray-600">{totalPoints} / 100 points this week</div>
            </div>
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <AlertCircle className="w-8 h-8 text-green-600 mx-auto mb-2" />
              <div className="font-medium">Team Work</div>
              <div className="text-sm text-gray-600">Everyone contributing!</div>
            </div>
          </div>
        </CardContent>
      </Card>
