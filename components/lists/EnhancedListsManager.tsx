        ...
        <div onClick={() => onSelect(list)}>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              {isProject && list.priority && (
                <div
                  className={`w-3 h-3 rounded-full ${getPriorityColor(list.priority)}`}
                ></div>
              )}
            </div>
            ...
            {isProject && (
              <div className="flex items-center justify-between mt-3">
                {list.status && (
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(list.status)}`}
                  >
                    {list.status}
                  </span>
                )}
          ...
          <CardContent>
            <div className="flex items-center justify-between text-sm text-gray-600 mb-3">
              <span>
                {list.item_count} {isProject ? 'tasks' : 'items'}
              </span>
                ...
                {list.timeSpent !== undefined && list.estimatedTime && (
                  <div className="flex items-center justify-between text-xs text-gray-600">
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        Time: {formatTime(list.timeSpent)} /{' '}
                        {formatTime(list.estimatedTime)}
                      </span>
                    </div>
                  </div>
                )}
...
function EnhancedListsManager() {
      ...
      setLists((prev) =>
        prev.map((list) =>
          list.id === parseInt(activeTimerProject)
            ? {
                ...list,
                timeSpent: (list.timeSpent || 0) + Math.floor(currentTime / 60)
              }
            : list
        )
      );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Family Lists & Projects
          </h1>
          <p className="text-gray-600">
            Organize everything your household needs - from groceries to major
            projects
          </p>
        </div>
      ...
      {lists.some(
        (list) => LIST_TYPES[list.type as keyof typeof LIST_TYPES]?.isProject
      ) && (
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {
                    lists.filter(
                      (list) =>
                        list.status === 'active' &&
                        LIST_TYPES[list.type as keyof typeof LIST_TYPES]
                          ?.isProject
                    ).length
                  }
                </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {Math.round(
                    lists.reduce(
                      (total, list) => total + (list.timeSpent || 0),
                      0
                    ) / 60
                  )}
                  h
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {Math.round(
                    lists
                      ...
                      .reduce(
                        (total, list) => total + (list.progress || 0),
                        0
                      ) /
                      Math.max(
                        1,
