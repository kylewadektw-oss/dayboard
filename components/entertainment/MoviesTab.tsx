...
const trendingMovies = [
  {
    ...
    duration: "2h 30m",
    poster: "🦝",
    description: "The final chapter in the beloved trilogy follows Peter Quill and the team...",
  ...
  },
  {
    ...
    duration: "2h 15m",
    poster: "🧜‍♀️",
    description: "The beloved Disney classic comes to life in this live-action adaptation...",
...
export const MoviesTab = memo(() => {
  const [activeSection, setActiveSection] = useState<'discover' | 'theaters' | 'watchlist' | 'nights'>('discover');
  ...
  const renderDiscoverSection = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {trendingMovies.map((movie) => (
            <div key={movie.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-700 mt-2 line-clamp-2">{movie.description}</p>
      ...
      {/* Streaming Services */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">📺 Now Streaming</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {['Netflix', 'Disney+', 'Prime Video', 'Apple TV+'].map((service) => (
            <div key={service} className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-4 text-center hover:shadow-md transition-shadow cursor-pointer">
              <div className="text-2xl mb-2">
                {service === 'Netflix' ? '🎬' : service === 'Disney+' ? '🏰' : service === 'Prime Video' ? '📦' : '🍎'}
              </div>
  ...
  const renderTheatersSection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Local Theaters & Showtimes</h3>
        <p className="text-gray-600">Find movie times at theaters near you</p>
      </div>

      <div className="grid gap-4">
        {['AMC Valley View 16', 'Regal Cinemas Downtown', 'Century Theaters Plaza'].map((theater, index) => (
          <div key={theater} className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h4 className="font-semibold text-gray-900">{theater}</h4>
                <p className="text-sm text-gray-600">📍 {index === 0 ? '2.1 miles away' : index === 1 ? '3.5 miles away' : '4.2 miles away'}</p>
              </div>
            ...
            <div className="space-y-2">
              {trendingMovies.slice(0, 2).map((movie) => (
                <div key={movie.id} className="flex items-center justify-between py-2 border-t border-gray-100">
                  ...
                  <div className="flex gap-2">
                    {movie.showtimes.slice(0, 3).map((time) => (
                      <button key={time} className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs hover:bg-blue-100">
                        {time}
                      </button>
                    ))}
                  </div>
  ...
  const renderWatchlistSection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Family Watchlist</h3>
  ...
  const renderMovieNightsSection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Planned Movie Nights</h3>
        <p className="text-gray-600">Upcoming family movie experiences</p>
      </div>

      <div className="space-y-4">
        {upcomingMovieNights.map((night) => (
          <div key={night.id} className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-semibold text-gray-900">{night.movie}</h4>
              <span className="text-sm font-medium text-purple-700">{night.date}</span>
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              {night.theater && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700">{night.theater} at {night.time}</span>
                </div>
              )}
      ...
      <div className="flex flex-wrap gap-2">
        {sections.map((section) => {
          ...
          return (
            <button
              key={section.id}
              onClick={() => setActiveSection(section.id as 'discover' | 'theaters' | 'watchlist' | 'nights')}
