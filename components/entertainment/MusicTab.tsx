...
const musicRecommendations = [
  {
    ...
    items: [
      { name: "Ed Sheeran", type: "Artist", reason: "Similar to recent plays", emoji: "🎸" },
      { name: "Uptown Funk", type: "Song", reason: "You love Bruno Mars", emoji: "🕺" },
      { name: "Feel-Good Pop Hits", type: "Playlist", reason: "Matches your vibe", emoji: "✨" }
    ]
  },
  {
    category: "Trending in Your Area",
    items: [
      { name: "Local Indie Rock", type: "Genre", reason: "Popular nearby", emoji: "🎸" },
      { name: "Coffee Shop Acoustic", type: "Mood", reason: "Trending locally", emoji: "☕" },
      { name: "NYC Hip-Hop Scene", type: "Scene", reason: "Local artists", emoji: "🏙️" }
    ]
  }
];

export const MusicTab = memo(() => {
  const [activeSection, setActiveSection] = useState<'discover' | 'playlists' | 'concerts' | 'recommendations'>('discover');
  ...
  const renderDiscoverSection = () => (
    <div className="space-y-6">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-gray-600 text-sm truncate">{song.artist} • {song.album}</p>
              ...
              <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                {Object.entries(song.streamingLinks).map(([platform]) => (
                  ...
                  >
                    ...
                    {platform === 'spotify' ? 'Spotify' : platform === 'apple' ? 'Apple' : 'YouTube'}
                  </button>
                ))}
              </div>
            </div>
      ...
      {/* Music Services */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">🎧 Connect Your Music</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={service.name} className={`bg-${service.color}-50 border border-${service.color}-200 rounded-lg p-4 text-center hover:shadow-md transition-shadow cursor-pointer`}>
  ...
  const renderPlaylistsSection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Family Playlists</h3>
        <p className="text-gray-600">Shared music collections for every occasion</p>
      </div>

      <div className="grid gap-4">
        {familyPlaylists.map((playlist) => (
          <div key={playlist.id} className="bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center gap-4">
  ...
  const renderConcertsSection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Upcoming Concerts</h3>
        <p className="text-gray-600">Live music events near you</p>
      </div>

      <div className="space-y-4">
        {upcomingConcerts.map((concert) => (
          <div key={concert.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start gap-4">
              <div className="text-3xl">{concert.emoji}</div>
              <div className="flex-1">
                <h4 className="font-semibold text-gray-900 text-lg">{concert.artist}</h4>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1">
              Get notified when your favorite artists announce shows in your area
            </p>
          </div>
        </div>
      </div>
  ...
  const renderRecommendationsSection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Music Recommendations</h3>
        <p className="text-gray-600">Discover new music tailored to your family&apos;s taste</p>
      </div>
        ...
        <div key={index} className="space-y-4">
          ...
          <div className="grid gap-3">
            {section.items.map((item, itemIndex) => (
              <div key={itemIndex} className="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
                <div className="text-2xl">{item.emoji}</div>
                <div className="flex-1">
                  <h5 className="font-medium text-gray-900">{item.name}</h5>
                  <p className="text-sm text-gray-600">{item.type} • {item.reason}</p>
                </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-purple-700 mt-1">
              Based on your listening history, we&apos;ll suggest new artists and songs your family might love
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {sections.map((section) => {
          ...
          return (
            <button
              key={section.id}
              onClick={() => setActiveSection(section.id as 'discover' | 'playlists' | 'concerts' | 'recommendations')}
