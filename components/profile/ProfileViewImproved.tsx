...
const HouseholdMapWidget = dynamic(() => import('@/components/dashboard/HouseholdMapWidget').then(m => ({ default: m.HouseholdMapWidget })), {
  ssr: false,
  loading: () => (
...
const PermissionsTab = dynamic(() => import('./PermissionsTab'), {
  ssr: false,
  loading: () => (
    <div className="bg-white rounded-xl border border-gray-200 p-6 animate-pulse" id="panel-permissions" role="tabpanel" aria-labelledby="tab-permissions">
      <div className="h-4 bg-gray-200 rounded w-32 mb-4"></div>
...
const DIETARY_OPTIONS = ['none','vegetarian','vegan','pescatarian','gluten_free','dairy_free','keto','paleo'];
const ALLERGY_OPTIONS = ['none','peanuts','tree_nuts','shellfish','fish','milk','eggs','wheat','soy','sesame'];
...
export default function ProfileViewImproved() {
  ...
  const [householdForm, setHouseholdForm] = useState<{ 
    ...
    name: '', household_type: 'family_household', address: '', city: '', state: '', zip: '', coordinates: undefined
  });
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const [householdResult, membersResult] = await Promise.all([
        ...
        supabase.from('profiles').select('id, name, preferred_name, role, family_role, avatar_url, last_seen_at').eq('household_id', profile.household_id).order('role', { ascending: false }).order('name')
      ]);
  ...
  const sortedMembers = useMemo(() => 
    [...householdMembers].sort((a, b) => {
      ...
      return (a.preferred_name || a.name || '').localeCompare(b.preferred_name || b.name || '');
    }), 
    [householdMembers]
  );
  ...
  const renderTabs = () => {
    ...
    return (
      <div className="mb-8" role="tablist" aria-label="Profile sections" onKeyDown={onKey}>
              ...
              <div className="space-y-8">
                  ...
                  <div className="space-y-6">
                    <div className="group">
                      ...
                      <p className="text-xl font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">
                        {profileForm.name || <span className="text-gray-400 italic">Not set</span>}
                      </p>
                    </div>
                    <div className="group">
                      <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                    ...
                    <div className="group">
                      ...
                      <p className="text-xl font-semibold text-gray-900 group-hover:text-pink-600 transition-colors">
                        {profileForm.date_of_birth ? new Date(profileForm.date_of_birth + 'T00:00:00').toLocaleDateString() : <span className="text-gray-400 italic">Not set</span>}
                      </p>
                    </div>
                    <div className="group">
                      <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                  ...
                  <div className="space-y-6">
                    <div className="group">
                      ...
                      <p className="text-xl font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">
                        {formatTimezone(profileForm.timezone) || <span className="text-gray-400 italic">Not set</span>}
                      </p>
                    </div>
                    <div className="group">
                      <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                      ...
                      <p className="text-xl font-semibold text-gray-900 group-hover:text-purple-600 transition-colors">
                        {LANGUAGE_OPTIONS.find(l => l.code === profileForm.language)?.label || 'English'}
                      </p>
                    </div>
                    <div className="group">
                      <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                      ...
                      <p className="text-lg leading-relaxed text-gray-800 group-hover:text-teal-600 transition-colors">
                        {profileForm.bio || <span className="text-gray-400 italic">No bio added</span>}
                      </p>
                    </div>
                  </div>
                </div>

                {(profileForm.dietary_preferences.length > 0 || profileForm.allergies.length > 0) && (
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-3">
                            {profileForm.dietary_preferences.map(pref => (
                              <span key={pref} className="px-4 py-2 bg-gradient-to-r from-green-100 to-emerald-100 text-green-800 rounded-full text-sm font-semibold shadow-sm hover:shadow-md transition-all">
                                {readable(pref)}
                              </span>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-3">
                            {profileForm.allergies.map(allergy => (
                              <span key={allergy} className="px-4 py-2 bg-gradient-to-r from-red-100 to-pink-100 text-red-800 rounded-full text-sm font-semibold shadow-sm hover:shadow-md transition-all">
                                {readable(allergy)}
                              </span>
                            ))}
                          </div>
                ...
                <div className="border-t border-gray-100 pt-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { key: 'notification_email', label: 'Email Notifications', enabled: profileForm.notification_email, color: 'blue' },
                      { key: 'notification_push', label: 'Push Notifications', enabled: profileForm.notification_push, color: 'purple' },
                      { key: 'notification_sms', label: 'SMS Notifications', enabled: profileForm.notification_sms, color: 'green' },
                      { key: 'notification_daycare_pickup_backup', label: 'Daycare Pickup Backup', enabled: profileForm.notification_daycare_pickup_backup, color: 'orange' }
                    ].map(item => (
                      <div key={item.key} className="flex items-center gap-4 p-4 bg-white/50 rounded-2xl border border-gray-100 hover:bg-white/80 transition-all">
                        <div className={`w-4 h-4 rounded-full flex items-center justify-center ${item.enabled ? 'bg-green-500' : 'bg-gray-300'} transition-all`}>
                          {item.enabled && <div className="w-2 h-2 bg-white rounded-full"></div>}
                        </div>
                        <span className={`text-sm font-semibold flex-1 ${item.enabled ? 'text-gray-900' : 'text-gray-500'}`}>
                          {item.label}
                        </span>
              ...
              <form onSubmit={(e)=>{e.preventDefault(); saveProfile().then(()=>setOverviewEditMode(false));}} className={`mt-2 pt-4 border-t space-y-8 rounded-md outline-offset-2 transition-shadow ${editHighlight ? 'outline outline-2 outline-purple-400/70 shadow-md' : ''}`} aria-label="Inline profile editor">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input value={profileForm.timezone} onChange={e=>handleProfileChange('timezone', e.target.value)} placeholder="e.g., America/New_York (EST), America/Los_Angeles (PST)" className="w-full px-3 py-2 rounded-md border border-gray-300 bg-white text-gray-900 focus:ring-purple-500 focus:border-purple-500" />
                    <p className="text-xs text-gray-500 mt-1">Use format: Region/City (Timezone abbreviation)</p>
                  </div>
                  <div>
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <button 
                    ...
                    onClick={profileDirty ? undefined : ()=>setOverviewEditMode(false)}
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-6 hover:shadow-2xl transition-all duration-300">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div>
                    <p className="font-bold text-gray-900 group-hover:text-emerald-600 transition-colors">Share Referral</p>
                ...
                >
                  ...
                  <div>
                    <p className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors">Household Settings</p>
                ...
                >
                  ...
                  <div>
                    <p className="font-bold text-gray-900 group-hover:text-orange-600 transition-colors">Complete Setup</p>
              ...
              {referralCode && (
                <div className="mt-6 pt-6 border-t border-gray-100">
                  ...
                  <p className="text-xs text-gray-500 mt-2 font-medium">Share with friends to get rewards!</p>
                </div>
              )}
              ...
              {profile?.profile_completion_percentage != null && (
                <div className="mt-6 border-t pt-4">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2 mb-1">
                    <div className="bg-gradient-to-r from-purple-600 to-pink-600 h-2 rounded-full transition-all duration-500" style={{ width: `${profile.profile_completion_percentage}%` }} />
                  </div>
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
            <div className="space-y-2">
              ...
              <p className="text-gray-600 text-lg font-medium">Manage your personal information, household, and preferences</p>
            </div>
          </div>
          {renderTabs()}
