...
import { Edit, Home, Phone, Mail, Globe, Clock, Shield, Bell, Settings, Save } from 'lucide-react';
import { Database } from '@/types_db';
import { toastHelpers } from '@/utils/toast';

// Lazy load heavy permissions tab to cut initial bundle size
...
// Added option constants (was causing build errors when undefined)
...
export default function ProfileView() {
  ...
  const [profileForm, setProfileForm] = useState<{ 
    name: string; preferred_name: string; phone_number: string; date_of_birth: string; timezone: string; language: string; bio: string; dietary_preferences: string[]; allergies: string[]; notification_email: boolean; notification_push: boolean; notification_sms: boolean; notification_daycare_pickup_backup: boolean; 
  }>({
    name: '',
    preferred_name: '',
    phone_number: '',
  ...
  // Permissions & household form state (single, clean definitions)
  ...
  // NEW: inline edit mode for overview
  const [overviewEditMode, setOverviewEditMode] = useState(false);
  const editFormRef = useRef<HTMLFormElement|null>(null); // NEW: ref to inline form
  const [editHighlight, setEditHighlight] = useState(false); // NEW: transient highlight
  const router = useRouter();
  const supabase = createClient();
  const tabChangeLoggedRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    const fetchHousehold = async () => {
      ...
      setLoading(false); // allow skeleton/content to render asap
    };
    if (!authLoading) fetchHousehold();
  }, [profile?.household_id, authLoading]);

  // Redirect to setup if profile incomplete (non-blocking skeleton already visible)
  useEffect(() => {
  ...
  useEffect(() => {
    if (tabChangeLoggedRef.current === activeTab) return;
    tabChangeLoggedRef.current = activeTab;
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      await refreshUser();
      try { appLogger?.info?.('[UI] Profile saved'); } catch {}
  ...
  const savePermissions = async () => {
    ...
    try {
      ...
      if (error) throw error;
      try { appLogger?.info?.('[Telemetry] permissions_save success'); } catch {}
      toastHelpers.success('Permissions updated');
      setPermissionsFeedback({type:'success', message:'Permissions updated'});
    } catch (e:any) {
      try { appLogger?.error?.('[Telemetry] permissions_save error', e?.message); } catch {}
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          household_type: householdForm.household_type as any,
        ...
        .eq('id', household.id);
      if (error) throw error;
      const { data: fresh } = await supabase.from('households').select('*').eq('id', household.id).single();
      setHousehold((prev: any) => ({ ...prev, ...(fresh || householdForm) }));
      try { appLogger?.info?.('[UI] Household saved'); } catch {}
  ...
  const canHouseholdSettings = permissionsForm.household_management === true;

  // prevent landing on forbidden tab
  useEffect(()=>{
    if (activeTab === 'household-settings' && !canHouseholdSettings) setActiveTab('overview');
  }, [activeTab, canHouseholdSettings]);

  const renderTabs = () => {
    const orderedTabs: { key: typeof activeTab; label: string; condition: boolean }[] = [
      { key: 'overview', label: 'Overview', condition: true },
      // user-settings tab removed (merged into overview inline edit mode)
      { key: 'household-settings', label: 'Household Settings', condition: canHouseholdSettings && !!household },
      { key: 'permissions', label: 'Permissions', condition: true }
    ];
    const visible = orderedTabs.filter(t => t.condition);
    ...
    return (
      <div className="flex flex-wrap items-center gap-2 mb-6 border-b border-gray-200" role="tablist" aria-label="Profile sections" onKeyDown={handleKeyDown}>
        {visible.map(t => (
          <button
            key={t.key}
            id={`tab-${t.key}`}
            ...
            className={`px-4 py-2 text-sm font-medium rounded-t-md focus:outline-none focus-visible:ring transition-colors ${activeTab===t.key ? 'bg-white text-gray-900 shadow-sm border border-gray-200 border-b-white -mb-px' : 'text-gray-600 hover:text-gray-900'}`}
          >{t.label}</button>
        ))}
      </div>
    );
  ...
  // Extracted permissions tab (lazy) so we remove original heavy JSX here.
  ...
  const renderSkeleton = () => (
    <div className="max-w-6xl mx-auto animate-pulse" aria-hidden="true">
      ...
      <div className="flex flex-wrap gap-2 border-b border-gray-200 mb-6">
        {['Overview','Household Settings','Permissions'].map(tab => (
          <div key={tab} className="h-8 w-28 bg-gray-200 rounded-t" />
        ))}
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              {Array.from({length:4}).map((_,i)=>(<div key={i} className="space-y-2"><div className="h-3 w-20 bg-gray-100 rounded" /><div className="h-4 w-32 bg-gray-200 rounded" /></div>))}
            </div>
            <div className="grid grid-cols-2 gap-4 pt-4">
              {Array.from({length:2}).map((_,i)=>(<div key={i} className="space-y-2"><div className="h-3 w-40 bg-gray-100 rounded" /><div className="flex gap-2 flex-wrap">{Array.from({length:3}).map((__,j)=>(<div key={j} className="h-6 w-16 bg-gray-200 rounded-full" />))}</div></div>))}
            </div>
            <div className="pt-6 border-t space-y-3">
              <div className="h-4 w-28 bg-gray-100 rounded" />
              <div className="flex gap-2 flex-wrap">{Array.from({length:4}).map((_,i)=>(<div key={i} className="h-6 w-20 bg-gray-200 rounded-full" />))}</div>
            </div>
          </div>
        </div>
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div className="space-y-2">
              {Array.from({length:4}).map((_,i)=>(<div key={i} className="h-9 w-full bg-gray-100 rounded" />))}
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-3">
            <div className="h-5 w-32 bg-gray-200 rounded" />
            <div className="space-y-2">
              {Array.from({length:5}).map((_,i)=>(<div key={i} className="h-4 w-40 bg-gray-100 rounded" />))}
            </div>
          </div>
        </div>
      </div>
  ...
  // Reuse existing tab renderers (overview/user/household) from original file
  // Inline (unchanged) overview rendering kept below for brevity
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="flex items-start gap-4 mb-6">
              ...
              <div className="flex-1">
                ...
                {profile.family_role && (
                  <span className="inline-block mt-3 px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-medium tracking-wide">
                    {profile.family_role.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                  </span>
                )}
              </div>
            </div>
            {/* ...existing code (about, contact, dietary, allergies, notifications) ... */}
            {overviewEditMode && (
              <form
                ref={editFormRef}
                tabIndex={-1}
                onSubmit={(e)=>{e.preventDefault(); saveProfile().then(()=> setOverviewEditMode(false));}}
                className={`mt-10 pt-8 border-t space-y-8 rounded-md outline-offset-2 transition-shadow ${editHighlight ? 'outline outline-2 outline-purple-400/70 shadow-md' : ''}`}
                aria-label="Inline profile editor"
              >
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">Edit Profile Details</h3>
                  <div className="flex gap-2">
                    ...
                    <button type="submit" disabled={savingProfile} className="inline-flex items-center gap-2 px-5 py-2 rounded-md bg-purple-600 text-white text-sm font-medium hover:bg-purple-700 disabled:opacity-60 focus:outline-none focus-visible:ring focus-visible:ring-purple-500">
                      {savingProfile ? <div className="animate-spin h-4 w-4 border-b-2 border-white rounded-full" /> : <Save className="w-4 h-4" />} {savingProfile ? 'Saving...' : 'Save Changes'}
                    </button>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input value={profileForm.timezone} onChange={e=>handleProfileChange('timezone', e.target.value)} className="w-full px-3 py-2 rounded-md border border-gray-300 bg-white text-gray-900 focus:ring-purple-500 focus:border-purple-500" placeholder="America/New_York" />
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea value={profileForm.bio} onChange={e=>handleProfileChange('bio', e.target.value)} rows={3} className="w-full px-3 py-2 rounded-md border border-gray-300 bg-white text-gray-900 focus:ring-purple-500 focus:border-purple-500" />
                  </div>
                  <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[
                      ...
                      <label key={item.key} className="flex items-center gap-2 text-sm font-medium text-gray-800">
                        <input type="checkbox" className="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded" checked={(profileForm as any)[item.key]} onChange={()=>handleProfileChange(item.key, !(profileForm as any)[item.key])} />
                        {item.label}
                      </label>
                    ))}
                  </div>
                  ...
                  {household.household_code && (
                    <div className="pt-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => { navigator?.clipboard?.writeText?.(household.household_code); try { appLogger.info('[UI] Copied household code'); } catch {} }}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="mt-6 border-t pt-4">
                ...
                <div className="flex items-center justify-between mb-2 text-xs font-medium text-gray-700">
                  <span>Progress</span><span>{profile!.profile_completion_percentage || 0}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2 mb-1">
                  <div className="bg-gradient-to-r from-purple-600 to-pink-600 h-2 rounded-full" style={{ width: `${profile!.profile_completion_percentage || 0}%` }} />
                </div>
  ...
  // Original userSettingsTab & householdSettingsTab kept (unchanged) below
  const userSettingsTab = null; // deprecated: merged into overview inline edit
  const householdSettingsTab = (
    !household ? <div className="text-sm text-gray-600" id="panel-household-settings" role="tabpanel" aria-labelledby="tab-household-settings">No household found.</div> : (
      <form onSubmit={(e)=>{e.preventDefault(); saveHousehold();}} className="space-y-6" id="panel-household-settings" role="tabpanel" aria-labelledby="tab-household-settings">
        ...
        <button type="submit" disabled={savingHousehold} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-md bg-purple-600 text-white font-medium hover:bg-purple-700 disabled:opacity-60 shadow focus:outline-none focus-visible:ring focus-visible:ring-purple-500">
          {savingHousehold ? <div className="animate-spin h-4 w-4 border-b-2 border-white rounded-full" /> : <Save className="w-4 h-4" />}
          {savingHousehold ? 'Saving...' : 'Save Household'}
        </button>
        {householdFeedback && <div className={`text-sm ${householdFeedback.type==='error'?'text-red-600':householdFeedback.type==='success'?'text-green-600':'text-gray-600'}`}>{householdFeedback.message}</div>}
      </form>
    )
  );

  const handleProfileChange = (field: string, value: any) => setProfileForm(p => ({ ...p, [field]: value }));

  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-gray-100 p-4 md:p-8 text-gray-900">
      {showLoading && (
        authLoading ? (
          <div className="min-h-[60vh] flex items-center justify-center" aria-label="Loading authentication state">
            <div className="text-center">
              <div className="animate-spin rounded-full h-24 w-24 border-b-2 border-purple-600 mb-4" />
              <p className="text-gray-600">Authenticating...</p>
            </div>
          </div>
        ) : (
          <div aria-label="Loading profile content" className="pb-16">{renderSkeleton()}</div>
        )
      )}
