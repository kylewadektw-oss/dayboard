...
const PermissionsTab = dynamic(() => import('./PermissionsTab'), {
  ...
  loading: () => (
    <div
      className="bg-white rounded-xl border border-gray-200 p-6 text-sm text-gray-600"
...
const appLogger =
  typeof window !== 'undefined'
    ? (window as WindowWithLogger).appLogger || console
    : console;
...
export default function ProfileView() {
  ...
  useEffect(() => {
    if (permissions) setPermissionsForm(permissions);
  }, [permissions]);
  ...
  useEffect(() => {
    if (lastLoggedTabRef.current !== activeTab) {
      try {
        appLogger.info(`[UI] Profile tab changed: ${activeTab}`);
      } catch {}
        ...
        .update({
          ...
          notification_preferences: {
            ...
            daycare_pickup_backup:
              profileForm.notification_daycare_pickup_backup
          }
        })
        .eq('id', profile.id);
  ...
  const canHouseholdSettings =
    permissionsForm.household_management === true && !!household;
  ...
  const renderTabs = () => {
    ...
    return (
      <div
        className="flex flex-wrap items-center gap-2 mb-6 border-b border-gray-200"
        role="tablist"
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-start gap-4 mb-6">
              <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-2xl font-semibold shadow">
                {(profile.preferred_name || profile.name || user?.email || 'U')
                  .charAt(0)
                  .toUpperCase()}
              </div>
            ...
            {!overviewEditMode && (
              <div className="flex justify-end mb-4">
                <button
                  ...
                  className="px-4 py-2 rounded-md border border-gray-300 bg-white text-gray-800 text-sm font-medium hover:bg-gray-50 shadow-sm flex items-center gap-2"
                >
            ...
            {household && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                  <Home className="w-5 h-5 text-purple-600" /> Household
                </h3>
                  ...
                  {household.household_code && (
                    <div className="pt-2">
                      ...
                      <div className="flex items-center gap-2">
                        <code className="px-2 py-1 rounded bg-gray-100 text-gray-800 text-xs font-mono border border-gray-200">
                          {household.household_code}
                        </code>
                        <button
                          type="button"
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full text-left p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors flex items-center gap-3 text-sm font-medium text-gray-700"
                >
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setHouseholdForm((f) => ({ ...f, address: e.target.value }))
            }
            className="w-full px-3 py-2 rounded-md border border-gray-300 bg-white text-gray-900 focus:ring-purple-500 focus:border-purple-500"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setHouseholdForm((f) => ({ ...f, city: e.target.value }))
            }
            className="w-full px-3 py-2 rounded-md border border-gray-300 bg-white text-gray-900 focus:ring-purple-500 focus:border-purple-500"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setHouseholdForm((f) => ({ ...f, state: e.target.value }))
            }
            className="w-full px-3 py-2 rounded-md border border-gray-300 bg-white text-gray-900 focus:ring-purple-500 focus:border-purple-500"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setHouseholdForm((f) => ({ ...f, zip: e.target.value }))
            }
            className="w-full px-3 py-2 rounded-md border border-gray-300 bg-white text-gray-900 focus:ring-purple-500 focus:border-purple-500"
          />
        </div>
        {householdFeedback && (
          <div
            className={`text-sm md:col-span-2 ${householdFeedback.type === 'error' ? 'text-red-600' : householdFeedback.type === 'success' ? 'text-green-600' : 'text-gray-600'}`}
          >
            {householdFeedback.message}
          </div>
        )}
        <div className="md:col-span-2 flex justify-end">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-md bg-purple-600 text-white font-medium hover:bg-purple-700 disabled:opacity-60 shadow focus:outline-none focus-visible:ring focus-visible:ring-purple-500"
          >
            {savingHousehold ? (
              <div className="animate-spin h-4 w-4 border-b-2 border-white rounded-full" />
            ) : (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-gray-100 p-4 md:p-8 text-gray-900">
      {showLoading && (
        <div aria-label="Loading profile content" className="pb-16">
          {renderSkeleton()}
        </div>
      )}
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Manage your personal information, settings, and permissions
              </p>
            </div>
