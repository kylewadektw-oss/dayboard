...
type PermissionBooleanKeys = Exclude<
  keyof UserPermissionsRow,
...
interface Props {
  ...
  setPermissionsForm: (
    updater: (prev: Partial<UserPermissionsRow>) => Partial<UserPermissionsRow>
  ) => void;
...
const PERMISSION_METADATA = [
  ...
  },
  {
    ...
    description: 'Access to profile settings and personal information',
    category: 'Core Features'
  },
  ...
  // Household Management
  {
    ...
    description: 'Manage household settings, members, and configurations',
    category: 'Household Management'
  },
  {
    key: 'user_management',
    title: 'User Management',
    description: 'Manage other users in the household',
    category: 'Household Management'
  },
  ...
  // Planning & Organization
  {
    key: 'meals',
    title: 'Meal Planning',
    description: 'Access to meal planning and grocery list features',
    category: 'Planning & Organization'
  },
  {
    key: 'lists',
    title: 'Lists & Tasks',
    description: 'Create and manage household lists and tasks',
    category: 'Planning & Organization'
  },
  {
    key: 'work',
    title: 'Work & Projects',
    description: 'Access to work tracking and project management',
    category: 'Planning & Organization'
  },
  {
    key: 'projects',
  ...
  // Entertainment & Lifestyle
  {
    key: 'sports_ticker',
    title: 'Sports Ticker',
    description: 'Access to sports updates and ticker information',
  ...
  // Advanced Features
  {
    key: 'ai_features',
    title: 'AI Features',
    description: 'Access to AI-powered recommendations and automation',
    category: 'Advanced Features'
  },
  {
    key: 'analytics_dashboard',
    title: 'Analytics Dashboard',
    description: 'View household analytics and insights',
    category: 'Advanced Features'
  },
  ...
  // Administration
  {
    ...
    description: 'Control which features are enabled for the household',
    category: 'Administration'
  },
  {
    key: 'global_feature_control',
    title: 'Global Feature Control',
    description: 'Administrative control over global feature settings',
    category: 'Administration'
  },
  {
    key: 'system_admin',
...
export default function PermissionsTab({
  ...
  const [debouncedPermissionSearch, setDebouncedPermissionSearch] =
    useState('');
  const [showPermissionDescriptions, setShowPermissionDescriptions] =
    useState(false);
  ...
  const permissionMeta = useCallback(
    (key: string) => {
    ...
    return ALL_PERMISSIONS.filter(
      (k) =>
        ...
        permissionMeta(k)
          ...
          .includes(debouncedPermissionSearch.toLowerCase())
    );
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          <h3 className="font-semibold text-gray-900 flex items-center gap-2">
            <Shield className="w-5 h-5 text-purple-600" /> Permissions
          </h3>
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <BulkPermissionToggle
              filteredKeys={filteredPermissionKeys}
        ...
        {permissionsFeedback && (
          <div
            className={`mb-4 p-2 rounded text-xs font-medium ${permissionsFeedback.type === 'error' ? 'bg-red-50 text-red-700 border border-red-200' : permissionsFeedback.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-gray-50 text-gray-700 border border-gray-200'}`}
          >
            {permissionsFeedback.message}
          </div>
        )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {group.keys.map((key) => {
                  ...
                  const active = Boolean(
                    (permissionsForm as Record<string, unknown>)[key]
                  );
                  return (
                    <div
                      key={key}
                      className={`flex items-start justify-between gap-3 p-4 rounded-lg border transition-colors ${active ? 'bg-purple-50 border-purple-200' : 'bg-gray-50 border-gray-200'}`}
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        {showPermissionDescriptions && description && (
                          <span className="text-xs text-gray-500 mt-1 line-clamp-3">
                            {description}
                          </span>
                        )}
                      </div>
                      <button
                        onClick={() =>
                          togglePermission(key as PermissionBooleanKeys)
                        }
                        aria-pressed={active}
                        className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center transition-all focus:outline-none focus-visible:ring ${active ? 'bg-purple-600 text-white' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}
                      >
                        {active ? (
        ...
        <div className="mt-8">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-md bg-purple-600 text-white text-sm font-medium hover:bg-purple-700 disabled:opacity-60 shadow focus:outline-none focus-visible:ring focus-visible:ring-purple-500"
          >
            {savingPermissions ? (
