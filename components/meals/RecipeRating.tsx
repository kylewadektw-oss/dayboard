...
export function RecipeRating({ 
  ...
  const getRatingLabel = (rating: number) => {
    switch (rating) {
      case -2: return 'Strongly Dislike';
    ...
    <div className="flex items-center space-x-1">
      ...
      <button
        ...
        className={`ml-2 transition-all hover:scale-110 ${
          recipe.is_favorite ? 'text-red-500' : 'text-gray-300 hover:text-red-400'
        }`}
      ...
      {/* Rating Label */}
      {showLabel && (
        <span className={`ml-2 font-medium ${getTextSize()} ${
          displayRating > 0 ? 'text-green-600' : 
          displayRating < 0 ? 'text-red-600' : 
          'text-gray-500'
        }`}>
...
export function RecipeRatingStats({ recipe, userRating, className = '' }: RecipeRatingStatsProps) {
  return (
    <div className={`flex items-center space-x-4 text-sm text-gray-600 ${className}`}>
      {/* Overall Rating */}
      <div className="flex items-center">
        <span className="font-medium text-yellow-600">{recipe.rating.toFixed(1)}</span>
