...
export function RecipeFiltersSheet({ open, onClose, value, onApply }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const filters: RecipeFilters = {
      ...
      includeIngredients:
        (fd.get('include') as string)
          ?.split(',')
          .map((s) => s.trim())
          .filter(Boolean) ?? [],
      excludeIngredients:
        (fd.get('exclude') as string)
          ?.split(',')
          .map((s) => s.trim())
          .filter(Boolean) ?? [],
  ...
  return (
    <dialog
      ref={ref}
      className="p-0 rounded-2xl shadow-2xl w-[min(720px,92vw)] border"
    >
      <form
        method="dialog"
        ...
        <div className="p-5 grid md:grid-cols-2 gap-6 max-h-[70vh] overflow-auto">
          ...
          <fieldset>
            ...
            <div className="flex flex-wrap gap-2">
              {DIETS.map((d) => (
                ...
                >
                  <input
                    defaultChecked={isChecked(value.diets, d)}
                    name="diet"
                    value={d}
                    type="checkbox"
                    className="hidden"
          ...
          <fieldset>
            ...
            <div className="flex flex-wrap gap-2">
              {ALLERGENS.map((a) => (
                ...
                >
                  <input
                    defaultChecked={isChecked(value.allergensExclude, a)}
                    name="allergen"
                    value={a}
                    type="checkbox"
                    className="hidden"
          ...
          <fieldset className="md:col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {CUISINES.map((c) => (
                ...
                >
                  <input
                    defaultChecked={isChecked(value.cuisines, c)}
                    name="cuisine"
                    value={c}
                    type="checkbox"
                    className="hidden"
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="text-sm">
              ...
              <input
                name="maxCookTime"
                defaultValue={value.maxCookTime ?? ''}
                type="number"
                min={1}
          ...
          <div className="md:col-span-2 grid md:grid-cols-2 gap-4">
            <label className="text-sm">
              ...
              <input
                name="include"
                defaultValue={value.includeIngredients?.join(', ') ?? ''}
                className="w-full h-10 px-3 rounded-lg border"
              />
            </label>
            <label className="text-sm">
              ...
              <input
                name="exclude"
                defaultValue={value.excludeIngredients?.join(', ') ?? ''}
                className="w-full h-10 px-3 rounded-lg border"
              />
            </label>
          </div>
          ...
          <div className="flex gap-6">
            <label className="flex items-center gap-2 text-sm">
              <input
                name="quickOnly"
                type="checkbox"
                defaultChecked={!!value.quickOnly}
              />
              Quick only (&lt;= 20m)
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                name="imagesOnly"
                type="checkbox"
                defaultChecked={!!value.imagesOnly}
              />
              Has image
            </label>
          </div>
