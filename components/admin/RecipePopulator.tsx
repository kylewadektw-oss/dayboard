...
export default function RecipePopulator() {
  ...
  const searchRecipes = async () => {
    ...
    setIsSearching(true)
    try {
      const response = await fetch(`/api/spoonacular?action=search&query=${encodeURIComponent(searchQuery)}&number=20`)
  ...
  const submitSelectedRecipes = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Successfully submitted ${selectedRecipes.size} recipes to approval queue!`)
      } else {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-600">Search and import recipes from Spoonacular API into approval queue</p>
      </div>
          ...
          <Card>
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                ...
                <Button variant="slim" onClick={getRandomRecipes} disabled={isSearching}>
                  Random
                </Button>
              </div>
              ...
              {selectedRecipes.size > 0 && (
                <div className="flex items-center justify-between bg-blue-50 p-3 rounded-lg">
                  ...
                  <Button onClick={submitSelectedRecipes} disabled={isSubmitting}>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1 mb-2">
                      {recipe.cuisines?.slice(0, 2).map((cuisine) => (
                        <span key={cuisine} className="px-2 py-1 bg-gray-100 text-xs rounded">
                          {cuisine}
                        </span>
                      ))}
                      {recipe.dishTypes?.slice(0, 1).map((type) => (
                        <span key={type} className="px-2 py-1 border border-gray-200 text-xs rounded">
                          {type}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span>Score: {recipe.spoonacularScore?.toFixed(0) || 'N/A'}</span>
